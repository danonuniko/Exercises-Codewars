...
public class Kata
{
    // Misma lógica que la usada en Java y misma sintaxis cambiando length por Length.
    public static int FindEvenIndex(int[] arr)
    {        
        for(int i = 0 ; i < arr.Length ; i++) 
        {
            int sumIzq = 0 , sumDerc = 0;
            for(int j = 0 ; j < i ; j++)                sumIzq += arr[j];
            for(int k = i+1 ; k < arr.Length ; k++)     sumDerc += arr[k];
            if(sumDerc == sumIzq)   return i;
        }
        return -1;
    }

    // Solución de la comunidad interesante.
    public static int FindEvenIndexCommunity(int[] arr)
    {
