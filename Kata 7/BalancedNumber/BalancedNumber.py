...
def balanced_num(number):
    ...
    if len(lista)% 2:
        izquierda = sum(x for x in lista[:len(lista) // 2])
        derecha = sum(x for x in lista[len(lista) // 2 + 1:])
        return "Balanced" if izquierda == derecha  else "Not Balanced"
    else:
        izquierda = sum(x for x in lista[:len(lista) // 2 - 1])
        derecha = sum(x for x in lista[len(lista) // 2 + 1:])
        return "Balanced" if izquierda == derecha  else "Not Balanced"
